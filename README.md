...
eighteen of the sets machine learning is taught with — MNIST, Fashion-MNIST,
KMNIST, EMNIST, CIFAR-10, CIFAR-100, the spoken digits of FSDD, Iris, the
Palmer penguins, Covertype, Adult, Mushroom, Letter Recognition, Optical
Digits, Wine, Breast Cancer Wisconsin, Dry Bean and Seeds — into ROOT files a
tree per class, fetched from whoever publishes them and carrying their licence
in the file, so a training loop can read them straight off a storage element
without anybody having to leave the tools they already use.
...
2912 tests, of which the great majority need no network, no KDC and no
